...
class Bajaj extends StatefulWidget {
  ...
  const Bajaj({super.key, this.username = '', this.email = ''});

  @override
  BajajState createState() => BajajState();
}

class BajajState extends State<Bajaj> {
  ...
  static const String _storageKey = 'bajaj_videos_v3';
  ...
  Future<void> _loadVideos() async {
    ...
    final defaultVideos = [
      {
        'url': 'assets/videos/bajaj1.mp4',
        'thumbnail': 'assets/images/bajaj1.png',
        'title': 'Bajaj Introduction',
        'isAsset': true,
        'isDefault': false,
      },
    ];
  ...
  Future<void> _loadBackendVideos() async {
    try {
      ...
      if (backendVideos != null && backendVideos.isNotEmpty && mounted) {
        setState(() {
          // Add backend videos that aren't already in the local list
...
// The FullScreenVideoPlayer class remains exactly the same as in yamaha.dart
